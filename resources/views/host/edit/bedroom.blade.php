            ...
            <div class="col-md-12">
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <form class="form-row" action="{{route('host.listing.update-bedroom',$product->slug)}}" method="post">
                            @method('patch')
                            @csrf
                            @php $n=0; @endphp
                            <div class="col-md-12" id="create_bedroom">
                                @foreach($bedInfos as $bedInfo)
                                    <div class="card-body bg-light form-row shadow-sm mb-3" id="bedroom-{{$n}}">
                                        <div class="form-group col-md-4">
                                            <label for="type_of_bed">{{__("Bed Type")}}<sup>*</sup></label>
                                            <input type="text" name="bedrooms[{{$n}}][type_of_bed]" class="form-control" value="{{$bedInfo->type_of_bed}}">
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label for="number_of_bed">{{__("Number of bed")}}<sup>*</sup></label>
                                            <input type="number" name="bedrooms[{{$n}}][number_of_bed]" class="form-control" value="{{$bedInfo->number_of_bed}}">
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label for="number_of_guest">{{__("Number of guests")}}<sup>*</sup></label>
                                            <input type="number" name="bedrooms[{{$n}}][number_of_guests]" class="form-control" value="{{$bedInfo->number_of_guests}}">
                                        </div>
{{--                                        <div class="col-md-12 mt-1 mb-2">--}}
{{--                                            <div class="text-left">--}}
{{--                                                <button type="submit" class="btn btn-red-danger" onclick="document.getElementById('bedroom-{{$n}}').remove()">{{__("Delete this room")}}</button>--}}
{{--                                            </div>--}}
{{--                                        </div>--}}
                                    </div>
                                    @php $n++; @endphp
